...
def get_requirements_file(base_requirements):
    """Get the appropriate requirements file based on distribution"""
    distro_id = get_distro_id()
    base_dir = os.path.dirname(base_requirements)
    base_name = os.path.basename(base_requirements).replace(".txt", "")
    
    # For openSUSE, use opensuse-specific requirements
    if distro_id in ["opensuse", "opensuse-leap", "opensuse-tumbleweed"]:
        opensuse_req = os.path.join(base_dir, f"{base_name}.opensuse.txt")
        if os.path.exists(opensuse_req):
            return opensuse_req
    ...
    if not os.path.exists(os.path.join(venv_path, "bin", "pip")):
        ...
        if requirements_file and os.path.exists(requirements_file):
            ...
            if result.returncode != 0:
                ...
                notify.send(
                    "HyDE PIP",
                    f"⚠️ Warning: Some dependencies failed to install.\n{error_msg[:500]}",
                )
                # Don't fail completely, just warn the user
            else:
                notify.send("HyDE PIP", "✅ Virtual environment created successfully")
        else:
            notify.send("HyDE PIP", "✅ Virtual environment created (no requirements file)")
    else:
        pass
...
def install_dependencies(venv_path, requirements_file):
    ...
    if not os.path.exists(venv_path):
        ...
        if result.returncode != 0:
            ...
            notify.send("HyDE PIP", "✅ Dependencies installed successfully")
...
def main(args):
    ...
    requirements_file = get_requirements_file(base_requirements_file)

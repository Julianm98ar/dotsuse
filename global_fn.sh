...
#--------------------------------#
# Detect distribution and distro-specific variables #
#--------------------------------#
detect_distro() {
...
pkg_installed() {
    ...
    opensuse | opensuse-leap | opensuse-tumbleweed)
        if zypper se --installed-only "${PkgIn}" &>/dev/null; then
            return 0
        fi
        ;;
    *)
        # Fallback: try pacman first, then zypper
        if command -v pacman &>/dev/null && pacman -Q "${PkgIn}" &>/dev/null; then
            return 0
        elif command -v zypper &>/dev/null && zypper se --installed-only "${PkgIn}" &>/dev/null; then
            return 0
        fi
        ;;
    esac

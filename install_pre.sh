...
# pacman (Arch-based distributions only)
...
opensuse | opensuse-leap | opensuse-tumbleweed)
    print_log -sec "ZYPPER" -stat "detected" "openSUSE uses zypper, skipping pacman configuration..."
    # zypper configuration can be added here if needed
    ;;
*)
    print_log -sec "PACKAGE-MGR" -stat "skipped" "Unknown distribution, package manager configuration skipped..."
    ;;
esac

# Additional repository configuration (distribution-specific)

case "${DISTRO_ID}" in
arch | manjaro | endeavouros)
    # Chaotic AUR for Arch-based distributions
    ...
    ;;
opensuse | opensuse-leap | opensuse-tumbleweed)
    # packman repository for openSUSE (multimedia codecs and additional packages)
    # TODO: Implement packman repository configuration for openSUSE
    # Uncomment and customize when packman support is ready:
    # print_log -sec "PACKMAN" -stat "pending" "Configuring packman repository for openSUSE..."
    # [ "${flg_DryRun}" -eq 1 ] || sudo zypper addrepo -cf https://packman.inode.at/suse/openSUSE_Leap_15.6/ packman
    # [ "${flg_DryRun}" -eq 1 ] || sudo zypper refresh
    print_log -sec "PACKMAN" -stat "pending" "packman repository configuration for openSUSE (TODO)"
    ;;
*)
    print_log -sec "REPOS" -stat "skipped" "Repository configuration not available for this distribution..."
    ;;
esac

    ...
    if [ ! -f /etc/sddm.conf.d/backup_the_hyde_project.conf ] || [ "${HYDE_INSTALL_SDDM}" = true ]; then
        ...
        *) sddmtheme="Corners" ;;
        ...
        if [[ ${flg_DryRun} -ne 1 ]]; then
            ...
            # Ensure Session is set to hyprland for Wayland
            if grep -q "^Session=" /etc/sddm.conf.d/the_hyde_project.conf; then
                sudo sed -i 's/^Session=.*/Session=hyprland/' /etc/sddm.conf.d/the_hyde_project.conf
            else
...
# openSUSE environment configuration for XDG portals
if [[ "${DISTRO_ID:-}" =~ opensuse ]]; then
    print_log -c "[ENVIRONMENT] " -b "detected :: " "openSUSE - installing portal path fixes"
    if [[ ${flg_DryRun} -ne 1 ]]; then
        # Fix XDG Desktop Portal paths using dedicated script
        if [ -f "${cloneDir}/Scripts/extra/fix_opensuse_portals.sh" ]; then
            bash "${cloneDir}/Scripts/extra/fix_opensuse_portals.sh"
        fi
        
        # Also update /etc/environment as fallback
        if ! sudo grep -q "/usr/libexec" /etc/environment 2>/dev/null; then
            echo "" | sudo tee -a /etc/environment > /dev/null
            echo "# HyDE openSUSE: XDG portal paths" | sudo tee -a /etc/environment > /dev/null
            echo "LD_LIBRARY_PATH=\"/usr/libexec:\$LD_LIBRARY_PATH\"" | sudo tee -a /etc/environment > /dev/null
        fi
    fi
fi
